...
public class Unwrapper {
    ...
    private void unwrapLines(PatchedData.Patch patch) {
        ...
        while (firstCol < patch.getToX() && firstRow < toY && Double.isNaN(patch.get(ORIENTATION, firstCol, firstRow))) {
            firstCol++;
            firstRow++;
        }
        ...
        for (int row = firstRow + 1; row < toY; row++) {
            double current = patch.get(ORIENTATION, firstCol, row);
            double last = patch.get(ORIENTATION, firstCol, row - 1);
        ...
        for (int row = firstRow; row < toY; row++) {
            patch.set(ORIENTATION, firstCol, row, patch.get(ORIENTATION, firstCol, row) + offsets[row - fromY]);
        }
        ...
        for (int row = firstRow; row < toY; row++) {
            ...
            for (int col = firstCol + 1; col < toX; col++) {
                double current = patch.get(ORIENTATION, col, row);
                double last = patch.get(ORIENTATION, col - 1, row);
                ...
                patch.set(ORIENTATION, col, row, current + offset);
            }
        }
    }
    ...
    private void unwrapSpirals(PatchedData.Patch patch) {
        ...
        while (visitedPixels < w * h) {
            ...
            if (!patch.isInRoi(x, y) || Double.isNaN(patch.get(ORIENTATION, x, y))) {
                visitedPixels++;
                patch.set(ORIENTATION, x, y, Double.NaN);
    ...
    private double unwrapOrientation(PatchedData.Patch patch, double offset, int x, int y, boolean[][] visited, int fromX, int fromY) {
        double reference = 0;
        double maxAbs = 0;
        double current = patch.get(ORIENTATION, x, y);

        int invalidPixels = 0;

        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                if ((i == 0 && j == 0) || Double.isNaN(patch.get(ORIENTATION, x + i, y + j)) || !visited[x + i - fromX][y + j - fromY]) {
                    invalidPixels++;
                    continue;
                }
                double val = patch.get(ORIENTATION, x + i, y + j);
        ...
        patch.set(ORIENTATION, x, y, current + offset);
        return offset;
    }
}
