...
public class LineDivergenceCalculator {
    ...
    public void calculateLineDivergence(FingerprintImage image) {
        ...
        if (lineDivergenceSmoothing != null) {
            ...
            for (PatchedData.Patch patch : image.getPatches()) {
                lineDivergenceSmoothing.smooth(patch, LINE_DIVERGENCE, smoothedLineDivergence);
            }
        ...
        for (int x = fromX; x < toX; x++) {
            for (int y = fromY; y < toY; y++) {
                if (!patch.isInRoi(x, y)) {
                    patch.set(LINE_DIVERGENCE, x, y, Double.NaN);
                    continue;
                }

                // Continuity adjustment. Apparently shifts the problem into regions where discontinuities just become less apparent.
                double val = ridgeFrequencyGradientX[x - fromX][y - fromY] * Math.cos(patch.get(ORIENTATION, x, y)) + ridgeFrequencyGradientY[x - fromX][y - fromY] * Math.sin(patch.get(ORIENTATION, x, y));
                patch.set(LINE_DIVERGENCE, x, y, val);
            }
        }
    }
}
