    ...
    public void estimateFrequency(FingerprintImage image) {
        ...
        if (hasWrongFrequency) {
            ...
            while (hasWrongFrequency && iteration < MAX_INTERPOLATION_ITERATIONS) {
                ...
                for (PatchedData.Patch patch : image.getPatches()) {
                    ...
                    frequencyInterpolation.interpolate(patch, PatchedData.DataType.RIDGE_FREQUENCY, frequenciesInterpolated, true);
        ...
        if (finalImageSmoothing != null) {
            ...
            for (PatchedData.Patch patch : image.getPatches()) {
                ...
                finalImageSmoothing.smooth(patch, PatchedData.DataType.RIDGE_FREQUENCY, smoothedFrequencies);
            }
        ...
        for (int x = fromX; x < toX; x++) {
            for (int y = fromY; y < toY; y++) {
                if (!patch.isInRoi(x, y) || Double.isNaN(patch.get(PatchedData.DataType.ORIENTATION, x, y))) {
                    patch.set(PatchedData.DataType.RIDGE_FREQUENCY, x, y, Double.NaN);
                    continue;
                }
                ...
                double orientation = patch.get(PatchedData.DataType.ORIENTATION, x, y);
                ...
                patch.set(PatchedData.DataType.RIDGE_FREQUENCY, x, y, frequency);
